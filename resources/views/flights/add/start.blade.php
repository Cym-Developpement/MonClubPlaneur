  ...
  <div class="col-md-12">
    <div class="row justify-content-center">
      @foreach(App\Models\sailplaneStartPrice::all() as $start)
        <div class="col-md-4" style="margin-bottom: 10px;"><button type="button" class="startSelect btn btn-block btn-primary" id="startSelect-{{ $start->id }}" data-name="{{ $start->name }}" data-price="{{ ($start->basePrice/100) }}" onclick="selectStart({{ $start->id }});">{{ $start->name }} ({{ number_format(($start->basePrice/100), 2).'€' }})</button></div>
      @endforeach
