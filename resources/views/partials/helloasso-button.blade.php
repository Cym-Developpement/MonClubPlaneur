{{-- Bouton de paiement HelloAsso officiel --}}
@php
    $amount = $amount ?? 0;
    $itemName = $itemName ?? 'Paiement';
    $containsDonation = $containsDonation ?? false;
    $buttonId = $buttonId ?? 'helloasso-pay-button';
...
<div class="helloasso-payment-container">
    @if($showAmount && $amountInCents > 0)
        <div class="helloasso-amount-display mb-3">
            <h4 class="text-center">
                Montant : <span class="text-primary">{{ number_format($amountInCents / 100, 2, ',', ' ') }} {{ $currency }}</span>
            </h4>
        </div>
    @endif

    <form id="{{ $formId }}" action="{{ route('helloasso.create-payment') }}" method="POST" class="d-inline">
        @csrf
