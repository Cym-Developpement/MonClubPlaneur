...
class ProductBillingController extends Controller
{
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            ...
            'quantity.*.integer' => 'Les quantités doivent être des nombres entiers.',
            'quantity.*.min'     => 'Les quantités doivent être d\'au moins 1.',
        ...
        foreach (User::whereIn('id', $data['users'])->get() as $user) {
            // Quantité propre à ce membre, 1 par défaut.
            $quantity  = max(1, (int) ($quantities[$user->id] ?? 1));
        ...
        $total = number_format($totalCts / 100, 2, ',', ' ');

        return redirect('/admin/facturationProduits')
            ->with('success', $count . ' membre(s) facturé(s) « ' . $label . ' » — '
                . $totalUnits . ' unité(s) à ' . number_format($unitCts / 100, 2, ',', ' ') . ' € '
